    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(model.getIsSelected()){
            holder.tvItem.background=
                ContextCompat.getDrawable(context,R.drawable.item_selected_circular_thin_color_accent_border)
            holder.tvItem.setTextColor(Color.parseColor("#212121"))
        }else if(model.getIsCompleted()){
            holder.tvItem.background=
                ContextCompat.getDrawable(context,R.drawable.item_completed_circular_thin_color_accent_border)
